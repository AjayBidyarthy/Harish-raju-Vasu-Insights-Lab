...
const FilterSidebar = ({ isOpen, onClose, onFilterChange }) => {
  ...
  const handleSensitivityChange = async (sensitivity) => {
    ...
    await fetchFilteredData(newSelectedSensitivities, selectedApprovalStates)
  }

  const handleApprovalStateChange = async (stateValue) => {
    ...
    await fetchFilteredData(selectedSensitivities, newSelectedStates)
  }

  const fetchFilteredData = async (sensitivities, approvalStates) => {
    try {
      const userStatusQuery = approvalStates.length > 0 
        ? approvalStates.join(',')
        : '1' 
      ...
      if (data.status === "success") {
        let filteredProducts = data.data.hits
        
        // Apply sensitivity filter if any selected
        if (sensitivities.length > 0) {
          filteredProducts = filteredProducts.filter(product => 
            sensitivities.includes(product.dataProductSensitivity)
          )
        }
